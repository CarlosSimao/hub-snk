 ...
 *
 * O formato tem armadilhas, todas silenciosas — nenhuma dá erro, só produz
 * dado errado ou lista vazia:
 *
 *  1. Os campos do recurso e do evento ficam sob `properties`, NÃO direto no
 *     nó. Lidos do nó, todos vêm vazios e a importação grava um snapshot de
 *     fantasmas.
 *  2. Todo valor sob `properties` vem embrulhado em `{ "$": "valor" }` — mas
 *     `allDay`, `start` e `end` são atributos diretos do `task`, em texto
 *     cru.
 *  3. `lane.task` é um OBJETO quando o recurso tem um único evento e um
 *     ARRAY quando tem vários. Sem normalizar, todo recurso com exatamente
 *     um evento some.
 *  4. Data vem `DD/MM/YYYY HH:mm`, cor vem `0xRRGGBB` e `allDay` vem
 *     `"true"`/`"false"`, enquanto os outros booleanos do Sankhya vêm
 *     `"S"`/`"N"`.
 */
 ...
 *
 * Texto, não `Date`: nesse formato a ordem lexicográfica é a ordem
 * cronológica, então `WHERE fim >= '2026-08-17 00:00:00'` funciona sem
 * conversão e sem fuso no meio.
 */
 ...
 *
 * O fallback para o próprio nó existe para o caso de algum ambiente
 * responder sem essa camada.
 */
...
export function parsearAgenda(bruto: unknown): AgendaImportada {
  ...
  // `status: "0"` significa que a própria chamada falhou; importar isso
  // gravaria vazio por cima do snapshot bom que já estava lá.
